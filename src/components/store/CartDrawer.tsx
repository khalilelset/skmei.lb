  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full sm:max-w-md bg-brand-white z-50 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out animate-in slide-in-from-right">
          ...
          {items.length === 0 ? (
            ...
            <div className="space-y-3 sm:space-y-4 lg:space-y-2">
              {items.map((item) => (
                <div
                  key={item.product.id}
                  className="flex gap-3 sm:gap-4 lg:gap-2.5 bg-brand-silver/10 rounded-xl p-3 sm:p-4 lg:p-2.5 transition-all duration-200 hover:bg-brand-silver/20"
                >
                  {/* Product Image */}
                  <div className="relative w-20 h-20 sm:w-24 sm:h-24 lg:w-16 lg:h-16 shrink-0 rounded-lg overflow-hidden bg-brand-white border border-brand-silver">
                    <Image
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Quantity Controls */}
                    <div className="flex items-center justify-between mt-3 lg:mt-2 gap-2">
                      <div className="flex items-center border border-brand-silver rounded-lg bg-brand-white">
                        <button
                          onClick={() =>
                            updateQuantity(item.product.id, item.quantity - 1)
                          }
                          className="p-2.5 sm:p-3 lg:p-1.5 hover:bg-brand-silver/30 transition-colors duration-200 active:scale-95 touch-manipulation rounded-l-lg"
                          aria-label="Decrease quantity"
                        >
                          <Minus className="w-4 h-4 lg:w-3 lg:h-3 text-brand-black" />
                        </button>
                        <span className="px-4 sm:px-5 lg:px-3 py-2 lg:py-1 text-sm font-semibold text-brand-black min-w-12 lg:min-w-8 text-center">
                          {item.quantity}
                        </span>
                        <button
                          onClick={() =>
                          ...
                          className="p-2.5 sm:p-3 lg:p-1.5 hover:bg-brand-silver/30 transition-colors duration-200 active:scale-95 touch-manipulation rounded-r-lg"
                          aria-label="Increase quantity"
                        >
                          <Plus className="w-4 h-4 lg:w-3 lg:h-3 text-brand-black" />
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        className="text-brand-red hover:bg-brand-red/10 transition-all duration-200 p-2.5 sm:p-3 lg:p-2 rounded-lg active:scale-95 touch-manipulation"
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="border-t border-brand-silver p-4 sm:p-5 lg:p-3 space-y-4 sm:space-y-5 lg:space-y-2 bg-brand-white">
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  {subtotal >= 50 && (
                    <div className="bg-green-50 border border-green-200 rounded-xl p-3 lg:p-2 text-center">
                      <p className="text-xs font-semibold text-green-700">🎉 You qualify for FREE shipping!</p>
                    </div>
                  )}

                  {/* Order Summary */}
                  <div className="space-y-2 lg:space-y-1 text-sm">
                    ...
                    <div className="flex justify-between font-bold text-brand-black text-base lg:text-sm pt-2 lg:pt-1 border-t border-brand-silver">
                      <span>Total</span>
            ...
            <div className="space-y-3 lg:space-y-1.5">
              <Link
                ...
                onClick={closeCart}
                className="w-full bg-green-600 hover:bg-green-700 text-white py-4 lg:py-2.5 rounded-lg font-semibold transition-all duration-200 flex items-center justify-center gap-2 active:scale-95 touch-manipulation min-h-[52px] lg:min-h-0"
              >
              ...
              <Link
                href="/store/checkout"
                onClick={closeCart}
                className="w-full bg-brand-red text-brand-white py-4 lg:py-2.5 rounded-lg font-semibold hover:bg-brand-red/90 transition-all duration-200 flex items-center justify-center gap-2 active:scale-95 touch-manipulation min-h-[52px] lg:min-h-0"
              >
                Proceed to Checkout
              ...
              <button
                onClick={closeCart}
                className="w-full bg-brand-silver/30 text-brand-black py-4 lg:py-2.5 rounded-lg font-semibold hover:bg-brand-silver/50 transition-all duration-200 text-center active:scale-95 touch-manipulation min-h-[52px] lg:min-h-0 flex items-center justify-center"
              >
                Continue Shopping
              </button>
            </div>
